...
public class WorkerNotificacoes : BackgroundService
{
    ...
    private void InitializeRabbitMq()
    {
        ...
        while (true)
        {
            ...
            _logger.LogInformation($"Tentativa {attempts} de ligação ao RabbitMQ (WorkerNotificacoes)...");
            ...
            try
            {
                ...
                _logger.LogInformation("Ligação ao RabbitMQ (WorkerNotificacoes) bem-sucedida! Filas declaradas.");
                break;
            }
            catch (Exception ex)
            {
                _logger.LogWarning($"Falha temporária de ligação ao RabbitMQ (WorkerNotificacoes): {ex.Message}. Aplicando backoff de {backoff}s antes de re-tentar...");
